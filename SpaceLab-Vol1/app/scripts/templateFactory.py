...
class TemplateBase:
   ...
   #/***************************************HOME***********************************/
    ...
    def buildLoginPage(self):
        ...
        self.btn_login = tk.Button(self.grid_area_3, text="Login",  width=10, height=1, bg="GREENYELLOW", border=None, fg=DARK_BLUE, command=lambda: self.pressLogin(self.input_email_institucional.get(), self.input_password.get(), db))
   ...
   #/***************************************HOME***********************************/
    ...
    def buildAddChoices(self, idTeacher):
        ...
        self.root.mainloop()   
    def pressRegister(self, d1, d2, d3, d4, db):
        email_institucional = d1
        password_1 = d2
        password_2 = d3
        recovery_answer = d4
        
        if not email_institucional or not password_1 or not password_2 or not recovery_answer:
            ...
            try:
                estudante.generateUserStudentDb()
                return messagebox.showinfo('Sucesso', 'Registrado com sucesso')
            except ValueError:
                return messagebox.showerror('Erro', 'Erro ao registrar usuário')
    def pressRegisterAdmin(self, d1, d2, d3, d4, db):
        email_institucional = d1
        password_1 = d2
        password_2 = d3
        recovery_answer = d4
    ...
    def pressLogin(self, d1, d2, db):
        ...
        if not self.email_institucional or not self.password:
            return messagebox.showerror('Erro', 'Todos os campos devem estar preenchidos')

        sL = SistemaLogin()
        email_suffix_to_method = {
            self.KEY_ADMIN: sL.checkLoginAdmin,
            self.KEY_STUDENT: sL.checkLoginStudent,
            self.KEY_TEACHER: sL.checkLoginTeacher
        }

        for suffix, method in email_suffix_to_method.items():
            if self.email_institucional.endswith(suffix):
                if method(db, self.email_institucional, self.password):
                    id = sL.checkIdUser(db, self.email_institucional)
                    if suffix == self.KEY_ADMIN:
                        return self.goToProfileAdmin(id)
                    elif suffix == self.KEY_STUDENT:
                        return self.goToProfileStudent(id)
                    elif suffix == self.KEY_TEACHER:
    ...
    def buildStudentRegister(self, db):
        self.buildTemplateBase()
        DARK_BLUE = "#242231"
        ...
        self.btn_project = tk.Button(self.grid_area_1, text="AppLab", bg="#476475", border=None, command=lambda: self.goToHome())
        self.btn_login = tk.Button(self.grid_area_1, text="Login", command=lambda: self.goToLogin())
        self.btn_registerStudent = tk.Button(self.grid_area_3, text="Matricula", command=lambda: self.goToRegister())
        ...
        self.btn_registerStudent = tk.Button(self.grid_area_3, text="CADASTRAR", bg="ORANGE", fg="WHITE", command=lambda: self.pressRegister(
            self.input_email_institucional.get(),
            self.input_password_1.get(),
            self.input_password_2.get(),
            self.input_recovery_answer.get(),
            db  # Certifique-se de que 'db' esteja definido em seu contexto
        ))
    ...
    def goToRegisterStudent(self, db):
        self.root.destroy()
        self.template = TemplateBase()
        self.template.buildStudentRegister(db)
